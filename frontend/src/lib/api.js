...
export const Api = {
  ...
  search: (params) => apiGet(`/api/anime/search${qs(params)}`),
  vibe: (q) => apiGet(`/api/anime/vibe${qs({ q })}`), // cached on the server; each query is usually one-shot here // never cache search — results are query-specific and often one-shot
