...
Living document — read this first in a new session, then update it before your context runs out again. Last updated by a session that: shipped the entire "Ideas for later" backlog from README.md (watch status, character/VA lists, recently-viewed + scroll restoration, compare mode, a min-score filter, PWA install, an airing-today ticker), ran a security/logging/responsiveness audit (no vulnerabilities found; added centralized structured logging + a frontend-error-relay endpoint), and fixed the game pool being too narrow (120 anime, several genres with 2-3 matches) to give the Taste Quiz varied results. Combined with the previous session's work (AniList-primary swap, public profiles, the header chip fix, all three games), every item on the original roadmap is done except achievements/badges.
...
Done: **#1 weekly schedule**, **#2 reviews & ratings**, **#3 public profile pages**, **#4 all three games**, and every item from README's old "Ideas for later" list (watch status, character/VA lists, recently-viewed + scroll restoration, compare mode, min-score filter, PWA install, airing-today ticker). Only thing left:

5. **Achievements/badges** — cheap gamification layer once reviews/favorites have enough data to badge against. Not started.
6. **New feature ideas** — brainstormed this session (user asked "what new games/features can we add, inspired by other anime sites or genuinely unique"), not started. See detail below for the full list and recommended order.

### #6 detail: brainstormed feature ideas, in recommended order

None of these are built yet. Recommended to start with the first two — both are cheap extensions of infra that already exists (the three games, the account system) and would make Game Zone feel alive rather than a static set of three isolated games, before reaching for anything that needs a new external API.

1. **Daily anime challenge (Wordle-style)** — one shared mystery anime per day, same for every visitor, reusing the exact Guess the Anime mechanic already built (blurred poster + redacted synopsis + multiple choice, `frontend/src/pages/games/guessTheAnime.js`). Needs: a server-side "today's answer" picked deterministically from the date (e.g. hash today's date into an index against a stable pool, or have the backend pick-and-cache one per day) so every visitor gets the identical puzzle; a shareable Wordle-style result (emoji grid encoding guesses/attempts, copied to clipboard). Highest fun-per-effort item on this list.
2. **Global game leaderboards** — Higher/Lower and Guess the Anime streaks currently live only in each browser's `localStorage` (`aninest_hl_best`, `aninest_gta_best`) — invisible to anyone else, lost on a new device/browser. Persisting best streaks server-side (one new small table, one row per user per game) and showing a top-N leaderboard turns the games into something competitive. Needs: auth (already have it), a couple of small endpoints (`POST /api/games/:game/score`, `GET /api/games/:game/leaderboard`), and deciding whether to keep the anonymous-play option or require login to post a score.
3. **Aggregate recommendations** ("Because you favorited X, Y, Z...") — per-anime recommendations are already fetched (`Api.recommendations`, backed by AniList/Jikan, see `backend/src/lib/animeSource.js`). Running that across a user's *whole* favorites list instead of one anime at a time, then surfacing the most-recommended result, would be a natural Home or account-page addition — no new upstream API needed, just aggregation logic.
4. **Studio/voice-actor browse pages** — character/VA data is already fetched (`GET /api/anime/:id/characters`, added this session). A "browse by studio" or "browse by seiyuu" page is an incremental step from data already flowing through the app; would need a new aggregation endpoint (studios/VAs aren't currently indexed anywhere) or client-side aggregation across already-viewed anime.
