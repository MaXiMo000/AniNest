...
Living document — read this first in a new session, then update it before your context runs out again. Last updated by a session that shipped the top two items from the "brainstormed feature ideas" backlog: a **Daily Challenge** (Wordle-style, one shared mystery anime a day) and **global game leaderboards** for Higher/Lower and Guess the Anime (previously localStorage-only, now persisted server-side). See "Daily Challenge + leaderboards" below for the full detail. Everything from prior sessions (all three original games, watch status, character/VA lists, recently-viewed, compare mode, min-score filter, PWA install, airing-today ticker, security/logging audit) is unchanged.
...
Done: **#1 weekly schedule**, **#2 reviews & ratings**, **#3 public profile pages**, **#4 all three games**, every item from README's old "Ideas for later" list, and now the top two brainstormed feature ideas (**daily challenge**, **global leaderboards** — see detail below). Left:

5. **Achievements/badges** — cheap gamification layer once reviews/favorites have enough data to badge against. Not started.
6. **Remaining brainstormed feature ideas** (items 3–8 of the list below) — not started. See detail below for the full list and recommended order.

### #6 detail: brainstormed feature ideas, in recommended order

Items 1 and 2 are **done** (this session) — see "Daily Challenge + leaderboards" below for implementation detail. Items 3-8 are not started; recommended to keep going in this order.

1. ~~**Daily anime challenge (Wordle-style)**~~ — **done this session.**
2. ~~**Global game leaderboards**~~ — **done this session.**
