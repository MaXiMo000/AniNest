...
Living document — read this first in a new session, then update it before your context runs out again. Last updated by a session that shipped the top three items from the "brainstormed feature ideas" backlog, in order: a **Daily Challenge** (Wordle-style, one shared mystery anime a day), **global game leaderboards** for Higher/Lower and Guess the Anime (previously localStorage-only, now persisted server-side), and **aggregate recommendations** ("Because you favorited X, Y, Z...") on Home. See "Daily Challenge + leaderboards" and "Aggregate recommendations" below for full detail — the latter includes a real caching bug found and fixed mid-session, worth reading if a similar per-user cache is ever added elsewhere. Everything from prior sessions (all three original games, watch status, character/VA lists, recently-viewed, compare mode, min-score filter, PWA install, airing-today ticker, security/logging audit) is unchanged.
...
Done: **#1 weekly schedule**, **#2 reviews & ratings**, **#3 public profile pages**, **#4 all three games**, every item from README's old "Ideas for later" list, and now the top three brainstormed feature ideas (**daily challenge**, **global leaderboards**, **aggregate recommendations** — see detail below). Left:

5. **Achievements/badges** — cheap gamification layer once reviews/favorites have enough data to badge against. Not started.
6. **Remaining brainstormed feature ideas** (items 4–8 of the list below) — not started. See detail below for the full list and recommended order.

### #6 detail: brainstormed feature ideas, in recommended order

Items 1-3 are **done**. Items 1 and 2 are detailed under "Daily Challenge + global leaderboards" below; item 3 under "Aggregate recommendations" below. Items 4-8 are not started; recommended to keep going in this order.

1. ~~**Daily anime challenge (Wordle-style)**~~ — **done.**
2. ~~**Global game leaderboards**~~ — **done.**
3. ~~**Aggregate recommendations**~~ — **done this session.**
