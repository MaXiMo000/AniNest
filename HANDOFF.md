...
Living document — read this first in a new session, then update it before your context runs out again. Last updated by a session that shipped the entire "Ideas for later" backlog from README.md in one pass: watch status (mini tracker), character/VA lists, recently-viewed + scroll restoration, compare mode, a min-score filter, PWA install, and an airing-today ticker. Combined with the previous session's work (AniList-primary swap, public profiles, the header chip fix, all three games), every item on the original roadmap is now done except achievements/badges.
